...
def detect_sponsorship(description):
    ...
    for phrase in non_sponsor_phrases:
        if phrase in desc:
            return "No"
    return "Yes"
...
def fetch_nhs_jobs(keyword, salary_from, max_pages, location_filter, progress_callback=None):
    ...
    for page in range(1, max_pages + 1):
        ...
        for job in listings:
            ...
            desc = getattr(job.description, "text", "")
...
# --- Extract Band, Number, and Job Type ---
def extract_number(text):
    match = re.search(r'(\d+)', text)
    return int(match.group(1)) if match else None
...
def get_job_details(url):
    try:
        ...
        if response.status_code == 200:
            ...
            hours_elem = soup.select_one('//*[@id="maincontent"]/div[2]/div[2]/ul/li[2]/div[3]/div[2]/ul/li[2]')
            job_type = hours_elem.get_text(strip=True) if hours_elem else "Unknown"

            return band, band_num, job_type
    except:
        pass
    return "Not found", None, "Unknown"

def apply_band_and_type_filter(df, band_from, band_to, job_type_selection, max_threads=10):
    st.info("Fetching job details (band + job type)... ⏳")
    with ThreadPoolExecutor(max_workers=max_threads) as executor:
        results = list(executor.map(get_job_details, df['URL']))

    df['Pay Band'], df['Band Num'], df['Job Type'] = zip(*results)
    df = df[df['Band Num'].notnull() & (df['Band Num'] >= band_from) & (df['Band Num'] <= band_to)]

    if job_type_selection == "Part time":
        df = df[df['Job Type'].str.lower().str.contains("part") & ~df['Job Type'].str.lower().str.contains("full")]
    elif job_type_selection == "Full time":
        df = df[df['Job Type'].str.lower().str.contains("full") & ~df['Job Type'].str.lower().str.contains("part")]
    elif job_type_selection == "Both":
        df = df[df['Job Type'].str.lower().str.contains("full") & df['Job Type'].str.lower().str.contains("part")]
...
# --- Streamlit App ---
st.set_page_config("NHS Job Search Tool", layout="wide")
st.title("🔍 NHS Job Scraper with Band, Type, and Sponsorship Filtering")
...
with st.sidebar.form("search_form"):
    ...
    band_from = st.number_input("Band From", value=3, min_value=1, max_value=9)
    band_to = st.number_input("Band To", value=8, min_value=1, max_value=9)
    pages = st.number_input("Pages to Search", value=5, min_value=1, max_value=100)
    location = st.text_input("Location (optional)", value="")

    job_type_selection = st.radio(
        "Filter by Job Type",
        ["All", "Part time", "Full time", "Both"],
        index=0
    )

    sponsor_filter = st.checkbox("Only show jobs with Sponsorship")
...
if submit:
    ...
    if df.empty:
        ...
        df = parse_salary_fields(df)
        df = clean_dates(df)
        df = apply_band_and_type_filter(df, band_from, band_to, job_type_selection)

        if sponsor_filter:
            df = df[df['Sponsorship'] == "Yes"]

        if df.empty:
            st.warning("No jobs matched all selected filters.")
        else:
            st.success(f"✅ Found {len(df)} job(s) matching your filters.")
            st.dataframe(df)
